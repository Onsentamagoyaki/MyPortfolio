...
public class PlayerMove : MonoBehaviour
{
    ...
    {
        if (!context.performed || _brinkDirection == Vector2.zero) return;
    ...
    private void _IsGround()//地面との設置判定をとる処理
    {
        ...
        new Vector2(0.8f, 1.0f), 0.0f, Vector2.down, distance: 0.05f, (1 << 10) + (1 << 13));//layer 10 ステージの足場やブロックのレイヤ　13 すり抜ける床のレイヤ

        if (hit.collider && Mathf.Abs(_rigid.velocity.y) <= 0.005f)
        {
            _bJump = false;
        }
        else _bJump = true;
    ...
    public void _OnSlipThrough(InputAction.CallbackContext context)//プレイヤーがすり抜ける床に乗った際に特定のキー入力ですり抜けるようにする処理
    {
        ...
        RaycastHit2D hit = Physics2D.BoxCast(transform.position - new Vector3(0.0f, transform.localScale.y / 2.0f, 0.0f),
        new Vector2(transform.localScale.x * 0.5f, 0.2f), 0.0f, Vector2.down, 0.5f, 1 << 13);//layer 13 すり抜ける床のレイヤ
